...
adds exactly one global, `window.CriticLoopPage`, and it carries eighteen keys: `splitChunks`,
`openFlags`, `isApplicable`, `renderFinding`, `expandAllControl`, `metricsStrip`, `metricLine`,
`shortenForDisplay`, `shortenQuoteForDisplay`, `counterLabel`, `appliedTotal`, `verdictLine`,
`apiErrorMessage`, `METRIC_ROWS`, `FINDINGS_OPEN_CAP`, `API_ERROR_CHARS`, `DRAFT_DISPLAY_CHARS`,
`QUOTE_DISPLAY_CHARS`. It was
